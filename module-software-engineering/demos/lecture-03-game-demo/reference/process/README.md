...
history — commits, changelog entries, reports, plans — and auditing the process
means reading that history.

## The files

| File | Concern | Prefix | Applies | Loaded |
|---|---|---|---|---|
| `development-rules.md` | how an operation begins, proceeds, and ends; who may change what; audit before plan; plan before build | DEV | every operation | always |
...
| `verification.md` | the kinds of verifier; mechanical vs judgment; when checks run; the gate; coverage policy; test discipline; fixtures; coverage vs conformance | VER | after every change; before every commit | always |
| `spec-audit.md` | how a specification's quality is assessed, generally and by kind of specification | AUD | before a plan; after an amendment; on request | on demand |
| `conops-audit.md` | rules specific to a concept of operations, including the full skeleton | AUDCON | with AUD, whenever the ConOps is audited | on demand |
...
is audited again (AUD; AUDCON for the ConOps). If it will build something, the
specifications it realizes are audited first (DEV-8), a plan is proposed and
approved (DEV-9), and only then is the realization made and verified (VER-3,
VER-4) and the result reported (RPT-2). A question the operation cannot settle
goes to `BACKLOG.md` (DEV-6); a question the repository can answer is answered by
reading it (DEV-10). The operation ends with a completion note (RPT-5) and a
commit named for it (DEV-7).
...
- **1.2** (2026-09-19) — VER-5 through VER-8 reflected in the map.
- **1.1** (2026-09-19) — DEV-8 through DEV-10 and the AUD appendix reflected in
  the map and the walkthrough; plans named among the history.

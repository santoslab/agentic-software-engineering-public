...
Reports cite `CONOPS.md` as §1 … §9 (policies in §4.2 by their bullet),
`SPECS.md` as §n.m, plans by step, and process rules as AUD-n, AUDCON-n, DEV-n,
VER-n, RPT-n. Reports are delivered in the conversation; the session that
produced a specification or an audit is exported to `transcripts/`; a report that
...
> 1. **Where:** `CONOPS-sketch.md` §4.2, third bullet. **Category:** ambiguous.
> **Quoted:** "Five in a row wins — across, down, or on a diagonal."
> **Recommended:** state that five *or more* consecutive marks win (an overline
> counts). **Status:** ruled: overlines count.
...
human or agent verifier; when the verifier is a test suite, which verification
obligations are claimed by at least one test and which are not (VER-8); any
question deferred to `BACKLOG.md`; and the commit message.
...
- **1.3** (2026-09-19) — RPT-5 also reports clause coverage against the
  verification obligations when the verifier is a test suite (VER-8).
- **1.2** (2026-09-19) — Binding rewritten for a code repository; `transcripts/`
  named; a game example for RPT-1.

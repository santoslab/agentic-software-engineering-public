...
## A note set under two specifications

The running example for this lecture is a repository that, at the start of class,
contains three files and no notes:

- `note-set-conops.md`, a concept of operations. One user keeps study notes in
  Markdown; an agent performs the structural operations (add a blank note, add a
...
rather than described in the abstract. Project 0 asks you to build a larger version
of the same thing; everything below applies there.
...
developer and leaves the rest free.
...
every file in `notes/` is a realization of the format specification. Later in the
demo, `check_notes.py` is also a realization of the format specification: it is
built to embody the rules, and it can be wrong about them.

**Conformance.** The relationship between S and R that holds when R satisfies every
property S states. It is a yes-or-no question about the pair: given this
...
different lengths, and different headings below the title can both conform.
...
be run against a stale copy of S. The section on verification returns to this.
...
realization of the same S. The demo script's final section makes the same point
about the specification itself: two students who run the demo and resolve its gaps
differently end with two realizations of the same intent. Lecture 02 takes this up.

## When conformance fails: change S or change R

A conformance check that fails reports a fact about the pair (S, R). It does not
say which side is wrong. Re-establishing conformance means changing one of them,
...
Each of these is a defect in S, found by attempting to plan a realization. The
resolutions are decided by the instructor, proposed by the agent as a numbered list
of amendments, approved, and applied: a scope statement (`index.md` is not a note;
R6 alone governs it), an extended R3 (the H1 equals the `title`; the field is
authoritative), and a new R7 (the filename is the slug of the title). The version
goes from 0.1 to 1.0.0 and the changelog records why: refinements surfaced during
implementation planning.

**Who decides.** In both cases a person decided which side moves. The working rules
...
The rule IDs are what make the three instruments comparable. All three cite R2,
R3, R4, and R5 for the same note. The operations document in the demo records the
split explicitly: a table listing which rules the script decides and which clauses
remain for the agent or the user.
...
A verification result is a claim made by an instrument, and the instrument can be
...
- **Wrong version of S.** In the demo's last segment the specification moves to
  ...
  implements.
- **The checker is a realization, and can fail conformance.** `check_notes.py` is
  ...
  inputs with known violations, or by comparing it with another instrument.
...
- **Attention.** The person is the instrument for judgment clauses, and for
...
is: which instrument, which version of S, which clauses. A report that states those
...
Described that way, it has the weakness of single-pass development that Royce's
1970 paper (the subject of the demo's first note) identified: information learned
during construction has nowhere to go. The demo is built to show five claims that
contradict the one-way description. Each follows from the terms above.

1. **A specification is a continuously maintained document of intent.** The format
...
2. **A specification is an abstraction of the realization.** R1–R8 describe every
   ...
   requires and guarantees, not how the agent carries it out.
...
4. **There are several specifications, and they must be consistent with each
...
5. **Information flows in both directions.** Planning produced R7 and the R3
   amendment. Writing the checker tightened the specification's front-matter
...
Generalized, it has six moves. The standalone handout
...
3. **Find gaps before planning.** Require a numbered list of every place the
   specifications are ambiguous, incomplete, or inconsistent: within one document,
...
5. **Plan.** Then a plan, and approval again.
6. **Realize with a verification gate.** Implement; after each step, check the
   affected artifacts against the specification and report findings, citing rule
   IDs, before declaring the step complete.

Move 3 is the one that matters most. It is requirement elicitation (Lecture 04)
pointed at the specifications rather than at you, and it is cheap: a few hundred

...
- **(c) Change of specified behavior** — a move after the game is over. The reference's `SPECS.md` 1.1.0 was silent; the silence passed through both plans and §9.5, so the engine accepted such a move and no test claimed otherwise. 1.2.0 adds the clause and records the ruling; the engine change and the test follow. Amendment, then realization.

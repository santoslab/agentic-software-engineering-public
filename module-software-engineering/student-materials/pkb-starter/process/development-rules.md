...
— adding a note, adding a Reference note from a source, removing a note, the
conformance check, and any other operation `pkb-conops.md` names — or a change
to a specification, or a change to a process document. Governing documents, in
authority order: `pkb-conops.md`, `pkb-format-spec.md`; derived:
`pkb-operations.md`. Deferred questions go to `BACKLOG.md`.
...
- **1.2** (2026-09-25) — Bound to the personal-knowledge-base repository: the binding block names the PKB's operations and governing documents. Rules unchanged.

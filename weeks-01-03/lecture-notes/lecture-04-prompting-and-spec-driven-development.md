...
## 6. Project 0 previewed: spec-driven development, on yourself

Everything above applies to a project with no compiler at all. **Project 0** is
a personal knowledge base (PKB) on agentic software engineering, organized
around your own interests, that you will maintain all semester. It is assigned
in the software-engineering module that follows this unit, after that module's
first two lectures have introduced specifications and the concept of
operations; this section is a preview of what you will build there.

The format is Google's **Open Knowledge Format (OKF)**: plain markdown files
with a YAML frontmatter block, two reserved files (`index.md`, a directory
listing; `log.md`, a chronological change history), and ordinary links, with a
rule worth noticing: *broken links are legal* — they mark knowledge you haven't
written yet. OKF's own conformance rule is light (one required field, `type`),
the format is Obsidian-viewable, and the spec explicitly targets knowledge
"authored by humans, generated by AI agents." The course restricts OKF to a
small profile of its fields — the brief says which and why — and a conformant
note in that profile is nothing more than this:
...
Four required fields, ordinary markdown below. A worked example bundle ships
with the brief; open it in Obsidian before designing your own.

The kickoff, in the module's brief, is this lecture applied to a markdown stack:
you write a sketch of what the knowledge base is for; a grill-me elicitation
turns the sketch into a concept of operations; a format specification (your
`type` vocabulary, folder layout, linking conventions, rules a verifier can
decide) is derived from it; Claude scaffolds the bundle from the specification
and seeds it with notes from these weeks' material, in your own words. The
stretch goal — a conformance verifier the agent can run — is principle 3
pointed at knowledge work.

The brief is `module-software-engineering/project-0-pkb-brief.md`. The PKB has
checkpoints in weeks 6, 10, and 15.
...
- **Project 0** is assigned in the software-engineering module. **Exercise 2** is

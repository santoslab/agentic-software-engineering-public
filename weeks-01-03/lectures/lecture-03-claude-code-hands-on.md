...
| 52–63 | Vague-prompt failure | Demo a deliberately vague prompt ("fix the AI, it plays bad"). The result is usually competent code rather than a wrong change, so judge the diff, not the gameplay. Two outcomes are possible: the agent adds a new strategy and leaves `random_move` intact, or it redefines, aliases, or delegates through `random_move` — silently changing a contract every caller and test depends on. Its summary mentions neither. Run `git diff computer_ai.py` on camera and make the judgement call then. Frame it as harness-independent: every agent decides something you did not ask for. Thursday picks this up as discussion plus hands-on model comparison in OpenCode (free tier) — not a re-run of this prompt. (The laptop micro-lab variant is not available this term — no install instructions shipped with L02.) |

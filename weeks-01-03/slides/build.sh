...
# PDF export. --no-stdin stops marp from waiting on stdin when there is no TTY
# (CI, or a shell run by a tool), where it would otherwise hang. The HTML output is standalone: present from it directly in a
# browser (press P for presenter view with the HTML-comment speaker notes).
for f in lecture-*.md; do
  [ -e "$f" ] || continue
  npx -y @marp-team/marp-cli@latest --no-stdin --allow-local-files "$f" -o "${f%.md}.pdf"
  npx -y @marp-team/marp-cli@latest --no-stdin --allow-local-files "$f" -o "${f%.md}.html"
